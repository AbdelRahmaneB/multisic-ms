...
export class PlayListService {
    ...
    create(playList: PlayList): Observable<EntityResponseType> {
        ...
        return this.http.post<PlayList>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    update(playList: PlayList): Observable<EntityResponseType> {
        const copy = this.convert(playList);
        return this.http.put<PlayList>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<PlayList>(`${this.resourceUrl}/${id}`, { observe: 'response'})
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    query(req?: any): Observable<HttpResponse<PlayList[]>> {
        const options = createRequestOption(req);
        return this.http.get<PlayList[]>(this.resourceUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<PlayList[]>) => this.convertArrayResponse(res));
    }
    ...
    private convertArrayResponse(res: HttpResponse<PlayList[]>): HttpResponse<PlayList[]> {
